...
export default function Analytics() {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      if (histRes.status === 'fulfilled') {
        ...
        const dates = histRes.value.data.map(h => h.date)
        if (dates.length >= 2) {
  ...
  const formatDateFull = (d) => new Date(d).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short', year: 'numeric' })
            ...
            {compLoading ? (
              ...
              <>
                ...
                <div className="space-y-4 mb-8">
                  {comparison.map((item) => {
                    const sc1 = STATUS_COLORS[item.status1]
                    const sc2 = STATUS_COLORS[item.status2]
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(value) => [`${value}%`, '% от нормы']}
                    />
