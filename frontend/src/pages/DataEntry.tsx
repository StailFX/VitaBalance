...
export default function DataEntry() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get<UserOut>('/profile/me').catch(() => ({ data: null })),
      api.get<LatestVitaminEntry[]>('/vitamins/entries/latest').catch(() => ({ data: [] })),
    ]).then(([vitRes, sympRes, profRes, entriesRes]) => {
      setVitamins(vitRes.data)
      setSymptoms(sympRes.data)
      setProfile(profRes.data?.profile ?? null)
      setLabValues(
        Object.fromEntries(
          entriesRes.data.map((entry) => [entry.vitamin_id, String(entry.value)])
        )
      )
      setLoading(false)
    })
  ...
  const handleSymptomSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      addToast('Анкета сохранена. Лабораторные значения сохраняют приоритет.', 'success')
      navigate('/analysis')
    } catch {
      addToast('Ошибка сохранения данных', 'error')
    } finally {
  ...
  return (
    <PageTransition>
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Mode toggle */}
        <div className="mb-8 grid w-full gap-1 rounded-2xl bg-gray-100 p-1.5 dark:bg-white/[0.03] sm:inline-grid sm:w-auto sm:grid-cols-2">
          <button
            onClick={() => setMode('lab')}
            className={`flex w-full items-center justify-center rounded-xl px-4 py-2.5 text-sm font-medium transition-all sm:px-6 ${
              mode === 'lab'
        ...
        {mode === 'lab' ? (
          <form onSubmit={handleLabSubmit} className="bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-200 dark:border-white/[0.08] shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <div className="space-y-4">
                {vitamins.map((v) => {
                  ...
                  return (
                    <div
                      key={v.id}
                      className="rounded-2xl border border-slate-200/70 bg-white/90 p-4 shadow-[0_14px_28px_-24px_rgba(15,23,42,0.16)] transition-colors hover:bg-white dark:border-white/[0.06] dark:bg-white/[0.02] dark:hover:bg-white/[0.04]"
                    >
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex min-w-0 items-start gap-3">
                          <div className={`surface-icon w-10 h-10 rounded-xl bg-gradient-to-br ${icon.gradient} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                        ...
                        <span className="shrink-0 rounded-full border border-gray-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-gray-500 dark:border-white/[0.08] dark:bg-white/[0.06] dark:text-gray-300">
                          {v.unit}
                        </span>
                      </div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full appearance-none rounded-xl border border-gray-200 bg-white px-4 py-3 text-base text-gray-900 outline-none transition-all placeholder-gray-400 focus:border-transparent focus:ring-2 focus:ring-primary-500 dark:border-white/[0.1] dark:bg-white/[0.05] dark:text-white dark:placeholder-gray-400"
                        />
                      </div>
                    </div>
                  )
              ...
              <div className="mt-6 pt-6 border-t border-gray-100 dark:border-white/[0.06]">
                <button type="submit" disabled={submitting} className="btn-primary inline-flex w-full items-center justify-center gap-2 rounded-2xl px-6 py-3 font-semibold text-white disabled:opacity-50 sm:w-auto sm:px-10">
          ...
          <form onSubmit={handleSymptomSubmit} className="bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-200 dark:border-white/[0.08] shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-100">
                Анкета самочувствия не затирает уже введенные лабораторные значения. Если по витамину есть анализ, он остается главным, а симптомы используются только как оценка дефицитов для отсутствующих показателей. Норму и избыток система определяет только по лабораторным данным.
              </div>
              {/* Symptom search */}
