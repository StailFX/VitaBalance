...
export default function AnalysisHistory() {
    ...
    api.get('/vitamins/history').then((res) => {
      ...
      if (res.data.length > 0 && res.data[0].entries) {
        ...
        const defVitamins = res.data[0].entries
          .filter((e: HistoryVitaminEntry) => e.status === 'deficiency')
          .map((e: HistoryVitaminEntry) => e.vitamin_name)
          .slice(0, 4)
        if (defVitamins.length > 0) {
          setSelectedVitamins(new Set(defVitamins))
        } else {
          setSelectedVitamins(new Set(res.data[0].entries.slice(0, 4).map((e: HistoryVitaminEntry) => e.vitamin_name)))
        }
      }
      setLoading(false)
    }).catch(() => {
  ...
  const chartData: ChartDataPoint[] = history.map(h => {
    const point: ChartDataPoint = { date: new Date(h.date).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit' }) }
