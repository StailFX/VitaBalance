...
export default function AnalysisResults() {
  ...
  // Radar chart data
  const radarData = analysis.filter(a => a.value !== null).map(a => ({
    vitamin: a.vitamin_name.replace('Витамин ', '').replace('(', '\n('),
    value: a.norm_max > 0 ? Math.round((a.value / a.norm_max) * 100) : 0,
    fullMark: RADAR_CHART_MAX,
  }))

  // Bar chart data
  const barData = analysis.filter(a => a.value !== null).map(a => ({
