...
export default function Dashboard() {
  ...
  const [profile, setProfile] = useState({ gender: '', age: '', height_cm: '', weight_kg: '' })
  ...
  const getGenderLabel = (g) => g === 'male' ? 'Мужской' : g === 'female' ? 'Женский' : 'Не указан'
            ...
            {editing ? (
              <form onSubmit={handleSave} className="grid grid-cols-2 gap-5">
                ...
                <div className="col-span-2 flex gap-3 pt-2">
                  <button type="submit" className="btn-primary text-white px-8 py-3 rounded-xl font-semibold">
                    Сохранить
                  </button>
                  <button type="button" onClick={() => setEditing(false)} className="px-6 py-3 rounded-xl text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/[0.06] transition-all font-medium">
                    Отмена
                  </button>
                </div>
              </form>
