...
import type { RecipeDetail as RecipeDetailType, RecipeShort, FavoriteOut } from '../types'
...
export default function RecipeDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setIsFav((favRes.data as FavoriteOut[]).some((f) => f.recipe_id === Number(id)))
      setLoading(false)
    }).catch(() => {
      setError(true)
      setLoading(false)
  ...
  const steps = recipe.instructions
    ...
    : []
  const heroImageSrc = resolveRecipeImageSrc(recipe.image_url)
  ...
  return (
    <PageTransition>
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-200 dark:border-white/[0.08] shadow-sm overflow-hidden">
          {/* Hero image */}
          <div className="relative h-56 bg-gradient-to-br from-primary-100 to-accent-100 dark:from-primary-900/30 dark:to-accent-900/30 sm:h-72">
            {heroImageSrc ? (
            ...
            {/* Time badge */}
            <div className="absolute bottom-3 left-4 flex items-center gap-2 sm:bottom-4 sm:left-6">
              <div className="flex items-center gap-2 rounded-xl bg-white/90 px-3 py-1.5 text-sm font-medium text-gray-700 backdrop-blur-sm dark:bg-white/[0.05] dark:text-gray-300 sm:px-4 sm:py-2">
          ...
          <div className="p-5 sm:p-8">
            <h1 className="mb-3 text-2xl font-bold text-gray-900 dark:text-white sm:text-3xl">{recipe.title}</h1>
            <p className="mb-6 text-base text-gray-500 dark:text-gray-300 sm:mb-8 sm:text-lg">{recipe.description}</p>
            ...
            <div className="mb-8">
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                {recipe.ingredients?.map((ing, i) => (
                  <div key={i} className="flex items-start gap-3 rounded-2xl bg-gray-50 p-3 transition-colors hover:bg-gray-100 dark:bg-white/[0.02] dark:hover:bg-white/[0.06]">
                    ...
                    <span className="min-w-0 flex-1 text-sm font-medium text-gray-700 dark:text-gray-300">{ing.product_name}</span>
                    <span className="shrink-0 pt-0.5 text-xs text-right text-gray-400 sm:text-sm">
              ...
              {relatedRecipes.map((r) => (
                ...
                >
                  <div className="relative h-32 bg-gradient-to-br from-primary-100 to-accent-100 dark:from-primary-900/30 dark:to-accent-900/30 overflow-hidden">
                    {resolveRecipeImageSrc(r.image_url) ? (
                      <img src={resolveRecipeImageSrc(r.image_url) ?? ''} alt={r.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
