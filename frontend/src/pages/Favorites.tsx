...
export default function Favorites() {
  ...
  useEffect(() => {
    api.get<FavoriteOut[]>('/favorites/').then((res) => {
      setRecipes(res.data)
      setLoading(false)
    }).catch(() => {
      setError(true)
  ...
  return (
    <PageTransition>
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-8">
          <span className="section-eyebrow inline-block px-4 py-1.5 rounded-full bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-semibold uppercase tracking-wider mb-4">
            Избранное
          </span>
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-300">
                {recipes.length} {recipeWord(recipes.length)} в избранном
              </p>
              ...
              {sortedRecipes.map((fav) => (
                <div key={fav.recipe_id} className="group bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-200 dark:border-white/[0.08] shadow-sm overflow-hidden card-hover">
                  <div className="relative h-48 bg-gradient-to-br from-rose-100 to-primary-100 dark:from-rose-900/30 dark:to-primary-900/30">
                    {resolveRecipeImageSrc(fav.recipe?.image_url) ? (
                      <img src={resolveRecipeImageSrc(fav.recipe?.image_url) ?? ''} alt={fav.recipe?.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
