...
import { MEAL_VISUALS, type MealType } from '../config/uiVisuals'
  ...
  return (
    <PageTransition>
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <span className="section-eyebrow inline-block px-4 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-semibold uppercase tracking-wider mb-4">
            План питания
          </span>
        ...
        {plan.length === 0 ? (
          ...
          <div className="space-y-6">
            {mealOrder.map((mealType) => {
              ...
              const config = MEAL_VISUALS[mealType]
              if (!meal) return null
              return (
                <div key={mealType} className="bg-white dark:bg-white/[0.03] rounded-3xl border border-gray-200 dark:border-white/[0.08] shadow-sm overflow-hidden card-hover">
            ...
            <div className="bg-white dark:bg-white/[0.03] rounded-2xl p-5 border border-gray-200 dark:border-white/[0.08] shadow-sm flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
